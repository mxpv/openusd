//! Decoded enums and the `UsdPreviewSurface` reader for the [UsdShade](super)
//! schema family.
...
// ── UsdPreviewSurface reader ─────────────────────────────────────────────
//
// `UsdPreviewSurface` is the portable shading model every USD consumer
// understands. The reader below resolves a Material's surface shader, checks
// it's a `UsdPreviewSurface`, and harvests each channel as either a scalar
// default or — when the input connects to a `UsdUVTexture` — the texture's
// `inputs:file` asset path. Renderer-specific shader dialects (MDL / OmniPBR /
// MaterialX `standard_surface`) are intentionally out of scope — consumers
// that need them can dispatch on [`read_shader_id`](super::read_shader_id).
