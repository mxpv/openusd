...
| Fallback values | `12.2.8` | :construction: | | `Attribute::get` / `get_at` fall through to the prim definition's fallback when composition resolves no value, including for a blocked attribute (§12.3.6); `Attribute::resolve_info` reports which tier answered<br>Remaining — import the OpenUSD schema data so the process registry has fallbacks to supply |
