...
| Ordered prim children (`primOrder` reordering) | `11.3.1` | :construction: | `0.4.0` | Partial: strongest `primOrder` is applied after the final merge. Left for full spec coverage: merge weakest-to-strongest, apply relocates per layer stack, and reapply `primOrder` after every merge. |
...
| [Stage queries](https://openusd.org/release/api/prim_flags_8h.html) (Active, Loaded, Defined, Abstract, Instance) | `11.5` | :white_check_mark: | `0.4.0` | `Stage::is_active`/`is_loaded`/`is_defined`/`is_abstract`/`is_instance`, `prim_status`, and `PrimPredicate` traversal filtering |
