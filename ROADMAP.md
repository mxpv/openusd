...
| [UsdShade](https://openusd.org/release/api/usd_shade_page_front.html) (materials, shaders) | :white_check_mark: | `0.5.0` | Trait-views behind `shade` (read + author): `Connectable`, `Input`, `Output`, `ConnectionTarget`, `ConnectedSources`, `ShadingAttribute`, `Input::value_producing_attributes` / `ProducerFilter`, Shader / NodeGraph / Material, `Material::compute_surface_source`, MaterialBindingAPI, and the UsdPreviewSurface reader<br>Remaining — render-context vector on the Material terminal queries; NodeGraph interface-consumer maps; Material base-material; `CoordSysAPI`; renderer shader dialects (MDL / MaterialX) |
