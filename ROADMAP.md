...
| Core schema types | `13.4` | :construction: | | The `usd` family in `usd::SCHEMAS`, registered by `SchemaRegistryBuilder::default`<br>Remaining — generated views for it; `usd::CollectionAPI` and `usd::ClipsAPI` are hand-written |
| [Value type names](https://openusd.org/release/api/class_sdf_value_type_name.html) | `13.3` | :white_check_mark: | `0.7.0` | `sdf::ValueTypeName`, `sdf::ValueKind`, `sdf::Dimensions`, resolved through one table by the parser, the writer and every setter<br>Remaining — default units (C++ `SdfDefaultUnit`); an unknown type's literal as `sdf::Value::UnregisteredValue` |
...
| [Schema codegen](https://openusd.org/release/tut_generating_new_schema.html) | `13.3` | :white_check_mark: | `main` | `openusd_build::configure` in a build script, `openusd::include_schema!` at the use site; `openusd_build::TokenEnum` for `allowedTokens`<br>Remaining — views for the core `usd` family |
