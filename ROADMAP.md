...
| [Value clips](https://openusd.org/release/api/_usd__page__value_clips.html) | `12.3.4` | :white_check_mark: | `0.5.0` | Explicit + template clips resolved during value resolution: `clips`/`clipSets` parsing, manifest gating, active-clip selection, stage-to-clip time mapping (incl. jump discontinuities), and clip strength below local opinions, with a `ClipsAPI` read/write view (UsdClipsAPI parity: per-set asset paths, active / times, manifest, primPath, interpolate-missing, and the template-clip fields)<br>Template clips (§12.3.4.1.3) expanded to explicit sets and retimed by layer offset<br>`interpolateMissingClipValues` interpolation and missing-value sentinels (manifest-default fallback / value block)<br>Remaining — `clipSets` ordering list-op authoring |
