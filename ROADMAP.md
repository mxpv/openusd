...
| [Scene graph instancing](https://openusd.org/release/glossary.html#usdglossary-instancing) | `11.3.3` | :white_check_mark: | `main` | Instances sharing a composition reuse one prototype, composed once; local opinions on an instance subtree are discarded<br>Prototype/instance queries and instance-proxy traversal; nested instances and connection-target remap into the queried instance; registry invalidated on composition change<br>Remaining — materialized `/__Prototype_N` namespace (currently alias-backed by the canonical instance), relationship-target remap (gated on §12.4), and parallel prototype composition |
...
| [UsdVol](https://openusd.org/release/api/usd_vol_page_front.html) (volumes) | :white_check_mark: | `main` | Reader + authoring behind `vol` feature: `Volume` (`field:<name>` relationships), `OpenVDBAsset` / `Field3DAsset` (shared FieldAsset attrs)<br>Remaining — the `ParticleField*` Gaussian-splat schemas |
...
| [UsdMedia](https://openusd.org/release/api/usd_media_page_front.html) | :white_check_mark: | `main` | Reader + authoring behind `media` feature: `SpatialAudio` (filePath / auralMode / playbackMode / startTime / endTime / mediaOffset / gain) and `AssetPreviewsAPI` (default thumbnail via `assetInfo.previews`) |
| [UsdUI](https://openusd.org/release/api/usd_u_i_page_front.html) | :white_check_mark: | `main` | Reader + authoring behind `ui` feature: `SceneGraphPrimAPI` (displayName / displayGroup), `NodeGraphNodeAPI` (node-editor layout), `Backdrop` |
| [UsdProc](https://openusd.org/release/api/usd_proc_page_front.html) | :white_check_mark: | `main` | Reader + authoring behind `proc` feature: `GenerativeProcedural` (`proceduralSystem`) |
...
| [Native instancing](https://openusd.org/release/glossary.html#usdglossary-instancing) (shared representation) | :white_check_mark: | `main` | Scene-graph instancing (see §11.3.3) — instances share one composed prototype, currently alias-backed by the canonical instance<br>Remaining — materializing `/__Prototype_N` as an independently composed namespace (C++-parity gap) |
