...
| [Scene graph instancing](https://openusd.org/release/glossary.html#usdglossary-instancing) | `11.3.3` | :white_check_mark: | `0.5.0` | Instances share one composed prototype; child names and descendants take only the shared subtree, discarding instance-namespace opinions. The `/__Prototype_N` namespace composes independently (proxies redirect onto it), so nested instances inside prototypes are modeled and relationship/connection targets remap (§12.4); variant selections key the prototype, prototype content populates under a population mask, and invalidation is change-targeted. |
...
| [Value clips](https://openusd.org/release/api/_usd__page__value_clips.html) | `12.3.4` | :white_check_mark: | `0.5.0` | Explicit + template clips resolved during value resolution: `clips`/`clipSets` parsing, manifest gating, active-clip selection, stage-to-clip time mapping (incl. jump discontinuities), and clip strength below local opinions, with a `ClipsAPI` read/write view (UsdClipsAPI parity: per-set asset paths, active / times, manifest, primPath, interpolate-missing, the template-clip fields, the whole-dict `clips` get/set, and the `clipSets` ordering list-op)<br>Template clips (§12.3.4.1.3) expanded to explicit sets and retimed by layer offset<br>`interpolateMissingClipValues` interpolation and missing-value sentinels (manifest-default fallback / value block)<br>Remaining — clip-manifest generation (`GenerateClipManifest` / `ComputeClipManifest`) |
...
| [UsdSkel](https://openusd.org/release/api/usd_skel_page_front.html) (skeletons, skinning) | :white_check_mark: | `0.5.0` | Trait-views behind `skel` feature (read + author on the same handle; `skel = ["geom"]`): SkelRoot / Skeleton as geom `Boundable`, SkelAnimation / BlendShape (incl. inbetween shapes) typed, SkelBindingAPI single-apply with namespace-inherited bindings — plus the time-independent object model (Topology, AnimMapper, SkeletonResolver, SkinningResolver, SkelAnimQuery, `discover_bindings`, pure-math LBS / blend shapes) building from the views<br>Remaining — stage-level interpolation of SkelAnimation time samples (left to the consumer) |
...
| [Edit targets](https://openusd.org/release/api/class_usd_edit_target.html) | :construction: | | `EditTarget` pairs a layer with a `PcpMapFunction` so authoring writes at the mapped spec path, with an RAII edit-context guard. Local, direct-variant, and arc-based targets (reference / payload / inherit / specialize, via `Stage::edit_target_for_node` / `Prim::edit_target_for_arc`, including instance-proxy redirection) are supported, with embedded relationship/connection targets re-mapped, root/session convenience targets, `compose_over` / `is_valid` / `is_null`, and cross-stage misuse rejection<br>Remaining — a real `PcpLayerStackIdentifier` key (cross-stage misuse currently caught by a coarser stage-identity tag) |
