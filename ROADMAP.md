...
| Relationship targets (raw + forwarded) | `12.4` | :construction: | | `main` — composed raw `targetPaths` via `Stage::relationship_targets` / `Relationship::get_targets`, folding list-op edits across every contributing layer and remapping through arcs (shares `Cache::property_targets` with connections); `has_authored_targets` introspection<br>Remaining — target forwarding (recursively chasing relationship-to-relationship chains) |
