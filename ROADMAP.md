...
| [Payload loading control](https://openusd.org/release/api/class_usd_stage_load_rules.html) | `10.3.2.2` | :white_check_mark: | `main` | `0.6.0` — `pcp::LoadRules` and `usd::LoadPolicy`, reached through `Stage::load` / `unload` / `set_load_rules` / `find_loadable`; `StageBuilder::load` sets the initial load set<br>`main` — `pcp::LoadRules::is_loaded_with_all_descendants` / `is_loaded_with_no_descendants`, and per-prim `usd::Prim::load` / `unload` |
