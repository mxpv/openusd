...
| [UsdMedia](https://openusd.org/release/api/usd_media_page_front.html) | :white_check_mark: | `main` | Reader + authoring behind `media` feature: `SpatialAudio` (filePath / auralMode / playbackMode / startTime / endTime / mediaOffset / gain)<br>Remaining — `AssetPreviewsAPI` (thumbnail previews via `assetInfo`); `timecode` attribute type in the USDA parser (startTime/endTime author in-memory; not yet parseable from text) |
| UsdUI, UsdProc | :construction: | | |
