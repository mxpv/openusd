...
| Composition errors (non-fatal) | `10.6` | :white_check_mark: | `0.3.0` | `pcp::CompositionDiagnostic`, read through `Stage::composition_errors`; missing/unreadable sublayers report raw from the loader and are filtered by `Stage::composition_errors` against the composed-stack effective set, so muting a branch suppresses its diagnostic and unmuting restores it<br>Remaining — precise target-diagnostic liveness (an orphaned interned target over-reports; see `pcp` module docs) |
...
| Property children include schema declarations | `11.3.2` | :white_check_mark: | `0.7.0` | `usd::Prim::property_names` unions authored names with the prim definition's, sorts the result and applies the composed `propertyOrder` (C++ `UsdPrim::GetPropertyNames`); `authored_property_names` keeps the layer-authored set alone |
...
| typeName resolution (from prim definition) | `12.2.2` | :white_check_mark: | `0.7.0` | `Attribute::type_name` takes the type its schema declares, consulting composition only for a property no schema declares (C++ `_GetAttrTypeImpl`); `variability` and `custom` resolve declaration-first the same way, and `get_metadata` routes those three fields through the same rules |
...
| Fallback values | `12.2.8` | :white_check_mark: | `main` | `Attribute::get` / `get_at` fall through to the prim definition's fallback when composition resolves no value, including for a blocked attribute (§12.3.6); `Attribute::resolve_info` reports which tier answered<br>`SchemaRegistry::builder` carries the core `usd` family, `openusd_schemas::schema_registry` the domain ones |
...
| [Value clips](https://openusd.org/release/api/_usd__page__value_clips.html) | `12.3.4` | :white_check_mark: | `0.7.0` | Explicit + template clips resolved during value resolution: parsing, manifest gating, active-clip selection, stage-to-clip time mapping (incl. jumps), clip strength below local opinions, with a full-parity `ClipsAPI` read/write view<br>Template clips (§12.3.4.1.3) expand to explicit sets, retimed by layer offset; `interpolateMissingClipValues` + missing-value sentinels<br>Manifest generation in `pcp::clip_manifest`, reached through `usd::ClipsAPI::generate_clip_manifest`; a set that authors no `manifestAssetPath` resolves through one synthesized from its clips, so every set is gated alike (§12.3.4.1.1.2)<br>`Attribute::time_sample_times` reports every clip's activation time alongside its in-window samples, isolating a clip from its neighbours (C++ `Usd_Clip::ListTimeSamplesForPath`)<br>Clip-sourced `asset` values resolve through `pcp::AssetSite::in_clip`; a `${VAR}` in the `clips` metadata's own asset paths evaluates through `PrimIndex::resolve_clip_sets` |
...
| Feature | Spec | Status | Version | Notes |
|---|---|---|---|---|
| [Schema registry](https://openusd.org/release/api/class_usd_schema_registry.html) | `13.3` | :white_check_mark: | `main` | `usd::SchemaRegistry` over `usd::FamilySource` pairs (schematics + manifest), with `usd::SchemaInfo`, `usd::PrimDefinition` and a `usd::PrimTypeInfo` interner; per-stage through `usd::StageBuilder::schema_registry` |
...
| Core schema types | `13.4` | :white_check_mark: | `main` | The `usd` family in `usd::SCHEMAS`, registered by `SchemaRegistryBuilder::default`, with generated views: `usd::CollectionAPI`, `usd::ClipsAPI`, `usd::ModelAPI`, `usd::ColorSpaceAPI`, `usd::ColorSpaceDefinitionAPI` |
...
| [Schema codegen](https://openusd.org/release/tut_generating_new_schema.html) | `13.3` | :white_check_mark: | `main` | `openusd_build::configure` in a build script, `openusd::include_schema!` at the use site; `openusd_build::TokenEnum` for `allowedTokens`<br>Per property: the reader, the creator, and the declaration itself as a builder (`<name>_attr_builder`) for authoring a value in the same edit. Per multiple-apply schema: `is_schema_property_base_name` and `instance_at_path`, which read an instance back out of a property path (C++ `Is<Name>Path`)<br>Doxygen documentation converted to Markdown in `openusd_build`'s `doc` |
...
| Feature | Spec | Status | Version | Notes |
|---|---|---|---|---|
| [CollectionAPI](https://openusd.org/release/api/class_usd_collection_a_p_i.html) | `15.1` | :white_check_mark: | `main` | Multi-apply `usd::CollectionAPI`, the generated view: `expansionRule` / `includeRoot` / `includes` / `excludes` / `membershipExpression` / `mode` read through the prim definition's fallbacks, so a schema's built-in collections carry what it declares (`LightAPI` light linking); authoring via `apply_collection` and `include_path` / `exclude_path` with edit minimization<br>Instance names are `SchemaRegistry::is_allowed_instance_name`'s, namespaced ones included; discovery through `CollectionAPI::get_all` and `CollectionAPI::instance_at_path`<br>Pattern-based expression mode: `sdf::PathExpression` + `sdf::path_expr::PathExpressionEval` with `sdf::path_expr::IncrementalSearcher` for traversal-order enumeration, `%_` composition and cross-arc mapping during field resolution, `usd::resolve_complete_membership_expression`, and the collection predicate library |
...
| [UsdPhysics](https://openusd.org/release/api/usd_physics_page_front.html) | :white_check_mark: | `main` | Trait-views behind `physics` feature (read + author on the same handle): all 8 typed prims (`Scene` / `CollisionGroup` / `Joint` + 5 joint subtypes via the `JointBase` interface), the 7 single-apply API schemas, and the multi-apply `DriveAPI` / `LimitAPI` (per-DOF instance, with `Get` / `GetAll`)<br>Collision groups: `physics::CollisionGroup::colliders_collection` reaches the built-in collection each group holds, and `physics::compute_collision_group_table` resolves every group's `filteredGroups` / `invertFilteredGroups` / `mergeGroupName` into a `physics::CollisionGroupTable` of which pairs collide |
...
| [UsdSemantics](https://openusd.org/release/api/usd_semantics_page_front.html) | :white_check_mark: | `main` | Schema view behind `semantics` feature: the multiple-apply `LabelsAPI`, whose instance name is the taxonomy its `semantics:labels:<taxonomy>` array labels a prim under, with `LabelsAPI::direct_taxonomies` / `inherited_taxonomies` (C++ `GetDirectTaxonomies` / `ComputeInheritedTaxonomies`)<br>Remaining — `UsdSemanticsLabelsQuery` |
...
| [Property stack queries](https://openusd.org/release/api/class_usd_resolve_info.html) | :white_check_mark: | `main` | `usd::ResolveInfo` / `usd::ResolveInfoSource` / `pcp::ResolveNode`, via `Attribute::resolve_info` / `resolve_info_at`<br>`Attribute::property_stack_at`; the stack queries and `Prim::prim_stack` return `usd::SpecSite`, one offset-bearing spelling where C++ has two<br>Site-major resolution over `pcp::value_resolve`'s one walk; value clips are placed by `pcp::ClipAnchor` and gated by `pcp::CompositionContext::may_have_clips`<br>`usd::ResolveInfo::spec_site` and `usd::ResolveInfo::weaker_sources` (C++ `GetNextWeakerInfo`), the chain `pcp::Composing` folds across the `default`, `timeSamples` and clip tiers and `pcp::close_composition` ends at the schema fallback |
