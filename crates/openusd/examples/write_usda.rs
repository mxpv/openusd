...
//!
//! Usage: `cargo run -p openusd --example write_usda -- out.usda`
...
fn main() -> openusd::Result<()> {
    ...
    let sphere_spec = data.create_spec(sdf::Path::new("/World/Sphere").unwrap(), SpecType::Prim);
    ...
    let attr_spec = data.create_spec(sdf::Path::new("/World/Sphere.radius").unwrap(), SpecType::Attribute);
