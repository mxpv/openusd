...
//! # Usage:
//! ```bash
//! cargo run -p openusd --example dump_usdc ./vendor/usd-wg-assets/full_assets/ElephantWithMonochord/SoC-ElephantWithMonochord.usdc
//! ```
...
fn main() -> Result<()> {
    ...
    let Some(path) = args.get(1) else {
        eprintln!("usage: cargo run -p openusd --example dump_usdc <FILE>.usdc");
        std::process::exit(2);
    };
