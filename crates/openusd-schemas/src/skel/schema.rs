...
//!
//! [`SkelRoot`] and [`Skeleton`] are [`geom::Boundable`](crate::geom::Boundable) prims;
...
use openusd::gf;
use openusd::sdf::{self, Value, Variability};
use openusd::tf;
...
use crate::common::{get_typed, get_with_api};

/// The enclosing scope beneath which skeletal posing and skinning apply (C++
/// `UsdSkelRoot`) — a [`geom::Boundable`](crate::geom::Boundable) prim. USD recommends authoring its
/// `extent` (via the inherited [`extent_attr`](crate::geom::Boundable::extent_attr))
...
/// a [`geom::Boundable`](crate::geom::Boundable) prim. `joints[i]` is the path-encoded name of joint
