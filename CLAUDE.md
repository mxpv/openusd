...
- **`usda/`** - Text format `.usda`: logos lexer + recursive-descent parser. `Parser` runs the grammar over a `Cursor` (one-token lookahead and the span bookkeeping behind `ParseError`'s line/column diagnostics); `TextWriter` emits.
