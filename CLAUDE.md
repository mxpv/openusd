...
- **`sdf/`** - Scene Description Foundations (C++ `Sdf`): core data types and the central `AbstractData` trait — the unified interface the text, binary, and archive readers all implement. Key types: `Value` (60+ variants), `AssetPath` (C++ `SdfAssetPath`), `Path`, `PathTable` (namespace-keyed map, C++ `SdfPathTable`), `ListOp`, schema `FieldKey`s, and `Layer` (C++ `SdfLayer`: identifier + a backing `AbstractData` wrapped in a recording `EditProxy` (C++ `SdfLayerStateDelegate`) under a copy-on-write `CowData` overlay). Spec authoring lives on the typed views — `PrimSpec::new` / `AttributeSpec::new` / `RelationshipSpec::new` (C++ `Sdf*Spec::New`) — which write through `Layer::data_mut`; the proxy records every mutation into a `ChangeList`. Edits write through to the backend immediately, or, inside a `Layer::transaction` (the `sdf::Transaction` guard, which derefs to the `Layer`), stage in the overlay and commit — or roll back when the guard drops — as one atomic transaction whose drained `ChangeList` drives composition invalidation; a multi-layer edit opens one transaction per layer and commits them together only once all have authored cleanly. `sdf::FileFormat` (C++ `SdfFileFormat`) is the pluggable per-format read/write seam — `usda::UsdaFileFormat`, `usdc::UsdcFileFormat`, and `usdz::UsdzFileFormat` wrap each format's reader/writer, found by extension via `find_by_extension` / `find_by_id` and gated by per-format `FileFormatCaps`; `Layer` persistence (`export` / `export_to_string` / `save`) routes through it. `sdf/expr.rs` is the `` `...` `` variable-expression engine (C++ `SdfVariableExpression`), re-exported as `sdf::expr` / `sdf::Expr`. `sdf/copy.rs` is the namespace-aware spec-copy primitive (C++ `SdfCopySpec`): `sdf::copy_spec` / `copy_spec_with` recursively copy a spec subtree, re-rooting embedded paths via `Value::remap_paths` (and the dropping variant `Value::filter_map_paths`) and driven by `should_copy_value` / `should_copy_children` policies (`CopyValue` / `CopyChildren`); `copy_spec_within` is the same-layer move primitive (scratch-`Data` round-trip) `usd::NamespaceEditor` builds on.
