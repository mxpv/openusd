...
- **`sdf/`** - Scene Description Foundations (C++ `Sdf`): core data types and the central `AbstractData` trait — the unified interface the text, binary, and archive readers all implement. Key types: `Value` (60+ variants), `AssetPath` (C++ `SdfAssetPath`), `Path`, `PathTable` (namespace-keyed map, C++ `SdfPathTable`), `ListOp`, schema `FieldKey`s, and `Layer` (C++ `SdfLayer`: identifier + a backing `AbstractData` under a copy-on-write `CowData` staging overlay). Spec authoring lives on the typed views — `PrimSpec::new` / `AttributeSpec::new` / `RelationshipSpec::new` (C++ `Sdf*Spec::New`) — which write through `Layer::data_mut`; every edit stages in the overlay. `ChangeList::from_overlay` (the change-record derivation in `sdf/change.rs`) classifies each staged `Patch` against the still-pristine base into the shape flags and authored-field set composition needs, and `Layer` accumulates that record on every flush. A crate-internal `sdf::Transaction` commits a batch atomically — flushing the overlay into the backend and accumulating the derived `ChangeList` — or rolls it back when the guard drops; a multi-layer edit opens one transaction per layer and commits them together only once all have authored cleanly. Edits made outside a transaction stay staged (still visible to reads, which merge staged-over-base) until the next flush. `sdf::FileFormat` (C++ `SdfFileFormat`) is the pluggable per-format read/write seam — `usda::UsdaFileFormat`, `usdc::UsdcFileFormat`, and `usdz::UsdzFileFormat` wrap each format's reader/writer, found by extension via `find_by_extension` / `find_by_id` and gated by per-format `FileFormatCaps`; `Layer` persistence (`export` / `export_to_string` / `save`) routes through it. `sdf/expr.rs` is the `` `...` `` variable-expression engine (C++ `SdfVariableExpression`), re-exported as `sdf::expr` / `sdf::Expr`. `sdf/copy.rs` is the namespace-aware spec-copy primitive (C++ `SdfCopySpec`): `sdf::copy_spec` / `copy_spec_with` recursively copy a spec subtree, re-rooting embedded paths via `Value::remap_paths` (and the dropping variant `Value::filter_map_paths`) and driven by `should_copy_value` / `should_copy_children` policies (`CopyValue` / `CopyChildren`); `copy_spec_within` is the same-layer move primitive (scratch-`Data` round-trip) `usd::NamespaceEditor` builds on.
...
- **`usd/`** - Composed stage API (C++ `Usd`): `usd::Stage` is a cheap `Rc<StageInner>` handle (C++ `UsdStageRefPtr`) that delegates composition to `pcp::IndexCache`. `StageBuilder` configures resolver, variant fallbacks, session layer, load set, and population mask (`open` from disk, `in_memory` for an anonymous root). Stage-tier authoring (`define_prim` / `create_attribute` / …) routes through the current `EditTarget`, with `EditContext` as the RAII guard; each authoring closure returns an `sdf::ChangeList` fed to `process_changes`. Composed handles — `Prim`, `Attribute`, `Relationship`, and the `schemas/` views — are `Clone` value types owning a `Stage` clone, and most methods mirror a C++ `UsdPrim` / `UsdAttribute` / … member by name. `Attribute::query` returns an `AttributeQuery` (C++ `UsdAttributeQuery`) that caches an attribute's resolved value source for repeated time-code reads, snapshotted against an `IndexCache` revision counter so edits invalidate it. `usd::SchemaBase` (`usd/schema.rs`) is the root of the schema-view hierarchy. `usd/notice.rs` (C++ `UsdNotice`) is the change-notification surface: `Stage::set_listener` installs one callback fired with a `Notice` (`ObjectsChanged` per edit, plus `EditTargetChanged` / `LayerMutingChanged` lifecycle notices). `usd/diff.rs` is the edit-replication surface built on it: `Stage::extract_diff` turns an `ObjectsChanged` into a transferable `Diff` — a flat list of `Edit` operations with inline values plus the producing edit target's mapping — and `Stage::apply_diff` replays it on a mirror per `ApplyMode`, verbatim onto a named layer or retargeted through the consumer's edit target (spec paths, embedded value paths, and time-sample keys all translated), dry-run on a scratch copy first so a failing diff leaves the stage untouched. `usd/editor.rs` is the namespace-editing surface (C++ `UsdNamespaceEditor`): `usd::NamespaceEditor` batches rename / reparent / delete edits and `apply`s them atomically across the local layer stack — each edit validated against the staged state, fixed up (relationship targets, connections, internal references) in place via `Value::filter_map_paths`, and realized through a relocate when the object composes across an arc — over one `sdf::Transaction` per layer. Public users import modules (`use openusd::{sdf, usd};`) rather than root-level re-exports.
